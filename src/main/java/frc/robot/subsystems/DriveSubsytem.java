...
public class DriveSubsytem extends SubsystemBase {
  ...
  private static PIDController thetaController = new PIDController(
  ...
  public MecanumDriveWheelPositions getWheelPositions() {
    return new MecanumDriveWheelPositions(frontLeftMotor.getWheelPostions(), frontRightMotor.getWheelPostions(),
        rearLeftMotor.getWheelPostions(), rearRightMotor.getWheelPostions());
  }
  ...
  public void periodic() {
    ...
    poseEstimator.updateWithTime(Timer.getFPGATimestamp(), pigeon.getRotation2d(), getWheelPositions());

    vision.getEstimatedGlobalPose(getPose2d()).ifPresent(pose -> {
      getPoseEstimator().addVisionMeasurement(pose.estimatedPose.toPose2d(),
    ...
    SmartDashboard.putNumber("Theta setPoint (Target))",
        Math.toDegrees(profiledThetaController.getSetpoint().position));
  ...
  public void setChassisSpeeds(double strafe, double drive, double turn, boolean needPID) {
    ...
    if (!needPID)
      chassisSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(xSpeed, ySpeed, turnSpeed, pigeon.getRotation2d());
    else
