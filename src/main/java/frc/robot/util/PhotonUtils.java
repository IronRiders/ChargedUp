...
public final class PhotonUtils {
   ...
   *     mounted 3 inches behind and 30 inches above the "origin" (usually bottom-center) of the
   *     robot, the translation component would be [-3 inches, 0 inches, 30 inches]. If the camera
   *     was pitched up 20 degrees (with "0" pitch being perpendicular to the robot), the rotation
   *     component would be [0 degrees, -20 degrees, 0 degrees].
  ...
  /**
   * Find the range(distance along ground) from the camera to the target through trigonometry using
   * the known target and camera heights. This method assumes the robot height and pitch/roll to be
   * zero.
   *
   ...
   * @param targetYaw The yaw of the target in the camera's 2d image, calculated from the camera's
   *     FOV. Positive values left.
   * @param targetPitch The yaw of the target in the camera's 2d image, calculated from the camera's
   *     FOV. Positive values down.
  ...
  public static Translation3d estimateCamToTargetTrl(
    ...
    return estimateCamToTargetTrl(
        0, new Rotation3d(), robotToCamera, targetHeight, targetYaw, targetPitch);
  }
  /**
   * Find the 3d camera-to-target translation(the target translation relative to the camera) through
   ...
   * at the same height as the camera can not be estimated and will return zero.
   *
   * @param robotHeight The height(z) of the robot's pose off the ground. The 3d robot pose is
   *     usually measured as the bottom-center of the drivetrain with height(z) == 0, unless on top
   *     of some element which elevates the robot.
   * @param robotPoseRot The 3d rotation of the robot, likely from a gyro or accelerometer. This is
   *     also usually zero [0, 0, 0]. The yaw of this rotation does not matter, only the roll and
   *     pitch.
   * @param robotToCamera The position of the camera relative to the robot. If the camera was
   *     mounted 3 inches behind and 30 inches above the "origin" (usually bottom-center) of the
   *     robot, the translation component would be [-3 inches, 0 inches, 30 inches]. If the camera
  ...
  public static Translation3d estimateCamToTargetTrl(
    ...
    var camToTargTrl =
        new Translation3d(1, Math.tan(targetYaw.getRadians()), -Math.tan(targetPitch.getRadians()));
    ...
    camToTargTrl =
        new Pose3d(camToTargTrl, new Rotation3d())
